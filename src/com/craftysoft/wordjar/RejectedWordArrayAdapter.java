...
public class RejectedWordArrayAdapter extends WordArrayAdapter {

	private static final int _layoutId = R.layout.wordlist_rejected_row;
	private RejectedWord[] _rejectedWords = null;

	
	public RejectedWordArrayAdapter(Context context, RejectedWord[] objects) {
		super(context, _layoutId, objects);

		_rejectedWords = objects;
	}
...
//				String h = r.getText().toString();
		...
		RejectedWord word = _rejectedWords[position];
		
		ViewHolder holder = (ViewHolder)rowView.getTag();		
		holder.col0.setText(word._word);
		holder.col1.setText(Integer.toString(word._todayCount));
		holder.col2.setText(Integer.toString(word._totalCount));
